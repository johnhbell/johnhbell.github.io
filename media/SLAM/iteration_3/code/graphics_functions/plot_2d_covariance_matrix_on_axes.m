function plot_2d_covariance_matrix_on_axes(ax,cov_matrix,...
    center_point,Zscore,line_thickness,line_color)
%PLOT_2D_COVARIANCE_MATRIX_ON_AXES Plots a 2D covariance matrix as an ellipse,
...
%
% This version of the function plots the ellipse on specified axes ax.
...
%% Plotting
plot(ax,[x1a x1b],[y1a y1b],'Color',line_color,'LineWidth',line_thickness);
hold(ax,'on');
plot(ax,[x2a x2b],[y2a y2b],'Color',line_color,'LineWidth',line_thickness);
r_plot_vec = linspace(0,2*pi,101);
plot(ax,x_param(r_plot_vec),y_param(r_plot_vec),'Color',line_color,...

function [] = part_h_fitted_scan_plotter(fitted_scan)
...
for it = 1:partition_count
    ...
    polarplot(angles_to_plot,rho_wall(angles_to_plot,...
...
for it = 1:partition_count
    polarplot(partitions{it}.angles,partitions{it}.dists,'.',...
...
%% Finishing Touches
title(['Fitted Scan at Time t = ' num2str(time,'%.1f') 's']);
pax = gca;
